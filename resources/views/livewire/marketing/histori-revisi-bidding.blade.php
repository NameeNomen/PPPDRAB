<div class="min-h-screen p-4 md:p-8 font-sans bg-[#FAFAFA] text-[#1A1A1A] print:p-0 print:m-0 print:block">
    ...
    <div class="max-w-7xl mx-auto space-y-6 print:m-0 print:p-0">

        <!-- Header Card -->
        <div class="print:hidden p-6 rounded-2xl shadow-xl border-2 border-[#E5E5E5] bg-white flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
        ...
        @if (session()->has('sukses'))
            <div class="print:hidden p-4 rounded-xl font-semibold flex items-center gap-3 shadow-xl border-2 border-[#2A402B]/40 bg-[#4A7256]/15 text-[#2A402B] text-xs tracking-wide uppercase">
        ...
        @if($view === 'project-list')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 print:hidden">
                @forelse($projects as $p)
                    ...
                    @endphp
                    <div wire:click="showCommits({{ $p->id }})" class="p-8 rounded-2xl border-2 border-[#E5E5E5] bg-white hover:border-[#4A7256] transition-all cursor-pointer group flex flex-col justify-between hover:-translate-y-0.5 hover:shadow-xl">
                        <div>
                        ...
                        <div class="mt-8 flex justify-between items-center border-t-2 border-[#E5E5E5] pt-4">
                            ...
                            <span class="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase px-3 py-1.5 rounded-lg border-2 {{ $biddingCurrent && $biddingCurrent->status_bidding === 'approved' ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/30' : 'bg-[#4A7256]/10 text-[#4A7256] border-[#4A7256]/30' }}">
        ...
        @elseif($view === 'commit-list')
            <div class="print:hidden p-4 rounded-t-2xl border-x-2 border-t-2 border-[#E5E5E5] bg-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div class="print:hidden rounded-b-2xl shadow-xl border-2 border-[#E5E5E5] bg-white overflow-x-auto">
                        ...
                        @forelse($historiCommits as $index => $c)
                            ...
                            <tr class="transition-all hover:bg-[#FAFAFA]">
                                ...
                                <td class="px-8 py-5 text-center">
                                    <button wire:click="showDetail({{ $c->id }})" class="inline-flex items-center gap-2 px-5 py-2 font-bold text-[10px] tracking-widest rounded-xl transition-all border-2 border-[#2A402B]/50 bg-white text-[#2A402B] shadow-sm hover:bg-[#4A7256] hover:text-white hover:border-[#4A7256] focus:outline-none focus:ring-4 focus:ring-[#4A7256]/30 hover:-translate-y-0.5">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start print:block print:m-0 print:p-0">
                ...
                <div class="print:hidden lg:col-span-1 space-y-6">
                    ...
                    <button onclick="window.print()"
                        class="print:hidden block w-full text-center py-4 bg-[#4A7256] hover:bg-[#354F37] text-white font-bold text-[11px] uppercase tracking-wider rounded-xl shadow-lg hover:shadow-xl hover:shadow-[#4A7256]/30 transition-all cursor-pointer hover:-translate-y-0.5 focus:outline-none focus:ring-4 focus:ring-[#4A7256]/30">

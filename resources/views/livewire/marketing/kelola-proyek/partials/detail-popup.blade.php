...
<div class="fixed inset-0 z-[100] bg-[#003057]/90 backdrop-blur-md flex flex-col" id="detailOverlay">
    <style>
        /* CSS udah dibikin realistis, gak usah gaya-gayaan nyamping */
        ...
        .paper-a4 { background-color: white; width: 210mm; min-height: 297mm; box-shadow: 0 10px 40px rgba(0,0,0,0.25); box-sizing: border-box; display: flex; flex-direction: column; flex-shrink: 0; }
    ...
    <main class="detail-scroll-container" id="detailScrollContainer">
        ...
        <div class="paper-a4 paper-surat">
            ...
            <table class="border border-black w-full text-[13px]">
                <tbody>
                    ...
                    <tr><td class="border border-black font-bold bg-gray-100 align-top">Catatan Tambahan</td><td class="border border-black whitespace-pre-line">{{ $detailProyek->deskripsi_proyek ?: 'Spesifikasi detail mengacu penuh pada dokumen RFQ utama yang dilampirkan.' }}</td></tr>
                </tbody>
            </table>
        ...
        {{-- HALAMAN LAMPIRAN --}}
        @if($rfq)
            <div class="paper-a4" style="padding:0; overflow:hidden;">
                <div class="bg-gray-800 text-white p-3 text-xs font-bold flex justify-between items-center"><span>DOKUMEN UTAMA RFQ: {{ $rfq->file_name }}</span><a href="{{ asset('storage/' . $rfq->file_path) }}" target="_blank" class="text-yellow-400 hover:underline">Layar Penuh</a></div>
                <iframe src="{{ asset('storage/' . $rfq->file_path) }}#toolbar=1&navpanes=0&scrollbar=1" class="w-full flex-1 border-none" style="height: calc(297mm - 40px);"></iframe>
            </div>
        @else
    ...
    <script>
        document.addEventListener('keydown', function(e) {
            if (document.getElementById('detailOverlay')) {
                // Scroll arrow udah gue matiin, ganti scroll atas bawah kalau pake keyboard
